#include "../../headers/intro/Curtain.h"

LPCWSTR Curtain::texturePath = nullptr;
LPDIRECT3DTEXTURE9 Curtain::texture = nullptr;
D3DCOLOR Curtain::colorKey = D3DCOLOR_XRGB(0, 0, 0);

Curtain::Curtain() {
	delayStart = static_cast<DWORD>(GetTickCount64());
}

RECTF Curtain::GetBoundingBox(int id) const {
	return hitBox.GetBoundingBox(0);
}

void Curtain::LoadTexture() {
	if (!texture) {
		HRESULT hResult;
...
void Curtain::ParseSprites(std::string line) {
	sprite.ParseSprites(line, texture, colorKey);
}

void Curtain::ParseHitboxes(std::string line) {
...
void Curtain::ParseData(std::string dataPath, std::string texturePath, D3DCOLOR colorKey, std::vector<std::string> extraData) {
...
void Curtain::Update(DWORD delta, std::vector<GameObject*>* objects) {
	GameObject::Update(delta);
	position += distance;

	if (GetTickCount64() - delayStart > delayTime) {
		velocity.y = -0.08f;		
	}
}

void Curtain::Render() {
	sprite.PlayAnimation("Curtain", position);
}

#include "../../headers/intro/SelectText.h"

LPCWSTR SelectText::texturePath = nullptr;
LPDIRECT3DTEXTURE9 SelectText::texture = nullptr;
D3DCOLOR SelectText::colorKey = D3DCOLOR_XRGB(0, 0, 0);

RECTF SelectText::GetBoundingBox(int id) const {
	return hitBox.GetBoundingBox(0);
}

void SelectText::LoadTexture() {
	if (!texture) {
		HRESULT hResult;
...
void SelectText::ParseSprites(std::string line) {
	sprite.ParseSprites(line, texture, colorKey);
}

void SelectText::ParseHitboxes(std::string line) {
...
void SelectText::ParseData(std::string dataPath, std::string texturePath, D3DCOLOR colorKey, std::vector<std::string> extraData) {
...
void SelectText::Update(DWORD delta, std::vector<GameObject*>* objects) {

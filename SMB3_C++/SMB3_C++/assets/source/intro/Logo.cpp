#include "../../headers/intro/Logo.h"

LPCWSTR Logo::texturePath = nullptr;
LPDIRECT3DTEXTURE9 Logo::texture = nullptr;
D3DCOLOR Logo::colorKey = D3DCOLOR_XRGB(0, 0, 0);

RECTF Logo::GetBoundingBox(int id) const {
...
void Logo::ParseSprites(std::string line) {
	sprite.ParseSprites(line, texture, colorKey);
}

void Logo::ParseHitboxes(std::string line) {
...
void Logo::ParseData(std::string dataPath, std::string texturePath, D3DCOLOR colorKey, std::vector<std::string> extraData) {
...
void Logo::Update(DWORD delta, std::vector<GameObject*>* objects) {

}

void Logo::Render() {
	sprite.PlayAnimation("3", position);
}

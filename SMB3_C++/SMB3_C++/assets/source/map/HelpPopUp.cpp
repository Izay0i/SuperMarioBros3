#include "../../headers/map/HelpPopUp.h"

LPCWSTR HelpPopUp::texturePath = nullptr;
LPDIRECT3DTEXTURE9 HelpPopUp::texture = nullptr;
...
void HelpPopUp::ParseSprites(std::string line) {
	sprite.ParseSprites(line, texture, colorKey);
}

void HelpPopUp::ParseHitboxes(std::string line) {
...
void HelpPopUp::ParseData(std::string dataPath, std::string texturePath, D3DCOLOR colorKey, std::vector<std::string> extraData) {
...
void HelpPopUp::Update(DWORD delta, std::vector<GameObject*>* objects) {

}

void HelpPopUp::Render() {
	sprite.PlayAnimation("Help", position);
}

void HelpPopUp::Release() {

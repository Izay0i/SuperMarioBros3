#include "../../headers/map/Cactus.h"

LPCWSTR Cactus::texturePath = nullptr;
LPDIRECT3DTEXTURE9 Cactus::texture = nullptr;
D3DCOLOR Cactus::colorKey = D3DCOLOR_XRGB(0, 0, 0);

RECTF Cactus::GetBoundingBox(int id) const {
	RECTF bound;
...
void Cactus::ParseSprites(std::string line) {
	sprite.ParseSprites(line, texture, colorKey);
}

void Cactus::ParseHitboxes(std::string line) {
...
void Cactus::ParseData(std::string dataPath, std::string texturePath, D3DCOLOR colorKey, std::vector<std::string> extraData) {
...
void Cactus::Update(DWORD delta, std::vector<GameObject*>* objects) {

}

void Cactus::Render() {
	sprite.PlayAnimation("Dance", position);
}
